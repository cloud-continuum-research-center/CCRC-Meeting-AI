...
async def saju_response(query: QueryRequest):
    ...
    또한 아래는 사주에서 사용되는 주요 개념 및 기운별 해석 정보입니다:
    "{similar_context}"

    이 정보를 바탕으로 사용자의 사주 성향을 아주 짧게 정리해주세요.  
    아래 조건을 지켜주세요:

    - 한 문장 또는 두 문장 이내로 작성
    - 사용자에게 직접 말하듯 자연스럽고 따뜻하게 설명
    - 주요 기운(예: 목, 화, 수 등) 중심으로 해석하고 간단한 음식/활동 조언 포함
    - 숫자/연월일은 반복하지 말고 해석 위주로만 말해주세요

    예시 출력:
    "당신은 금(金)의 기운이 강한 편이에요. 예리하고 이성적인 성향으로, 찬 음식은 피하는 게 좋아요."
    """
